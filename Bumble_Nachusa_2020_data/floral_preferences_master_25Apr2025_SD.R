...
flow <- bumble_final %>% 
  ...
  summarize(total_bumble_visits = sum(abun))

flow_b <- bumble_final %>% 
  group_by(flower_species) %>% 
  summarize(total_bumble_visits = sum(abun),
            all_visits = sum(flow$total_bumble_visits),
            prop_bumble_visits = total_bumble_visits / all_visits) %>% 
  mutate(across(starts_with("prop"), ~round(., digits = 3))) %>% 
  arrange(desc(total_bumble_visits)) # proportion of bumble bee visits to each species

flow_f <- floral2020 %>% 
  group_by(flower_species, date, site) %>% 
  summarize(total_surveys = count(floral2020))
